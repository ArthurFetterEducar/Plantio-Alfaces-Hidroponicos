...
void LigaPeliterF(float temp) {
  ...
  */
    ...
    if (temp >= 34)
      ledcWrite(ledChanne2 , 150);
    if (temp > 30 && temp <= 34)
      ledcWrite(ledChanne2 , 100);
    if (temp > 26 && temp <= 30)
      ledcWrite(ledChanne2 , 50);
...
void Reabastecimento_Nutriente() {
  ...
  for (pos = 145; pos >= 80; pos -= 1) { // goes from 180 degrees to 0 degrees
    myservo.write(pos);              // tell servo to go to position in variable 'pos'
    delay(20);                       // waits 15 ms for the servo to reach the position
  }
  
  for (pos = 80; pos <= 140; pos += 1) { // goes from 0 degrees to 180 degrees
